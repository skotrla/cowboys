    ...
    case 'sellers':
        ...
        if hash[0] != hashlib.sha256((user[0]+st.secrets['MYKEY']).encode()).hexdigest() or s != 'Y':
            ...
            with form:
                ...
                min_qty = st.number_input("Min Qty",min_value=0,value=0,step=1,disabled=True)
                max_qty = st.number_input("Max Qty",min_value=0,value=0,step=1,disabled=True)
                low_price = st.number_input("Low Price (ea)",min_value=0, value=0,step=1,disabled=True)
                high_price = st.number_input("High Price (ea)",min_value=0, value=0,step=1,disabled=True)
    ...
    case 'buyers':
        ...
        if hash[0] != hashlib.sha256((user[0]+st.secrets['MYKEY']).encode()).hexdigest():
            ...
            with form:
                ...
                min_qty = st.number_input("Min Qty",min_value=0,value=0,step=1,disabled=True)
                max_qty = st.number_input("Max Qty",min_value=0,value=0,step=1,disabled=True)
                price = st.number_input("Price (ea)",min_value=0, value=0,step=1,disabled=True)
                parking_included = st.selectbox("Parking Included?",['N','Y'])
                details = st.text_input("Details",value='')
                submit = st.form_submit_button("Submit")
                if submit:        
